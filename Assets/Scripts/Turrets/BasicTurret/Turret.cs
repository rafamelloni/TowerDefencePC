...
public class Turret : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Si hay un enemigo v�lido
        if (closestEnemy != null && closestDistance < normalTurretData.rotationDistance)
        {
            Transform enemyTransform = closestEnemy.transform;

            // Calcular la direcci�n hacia el enemigo (ignorando la diferencia en Y)
            Vector3 direction = enemyTransform.position - transform.position;
            direction.y = 0; // Asegurar que la rotaci�n ocurre solo en el plano horizontal

            // Evitar rotaciones innecesarias
            if (direction.magnitude > 0.1f)
            {
                // Calcular la rotaci�n hacia el enemigo
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(transform.position, 2f);  // Dibuja la esfera de colisi�n con radio de 2 unidades
    }
